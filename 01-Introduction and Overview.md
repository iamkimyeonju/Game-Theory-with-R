...
- **Extensive Form** Includes timig of moves (later in course)
...
- **Utility function** for player $i$:
  - $u_{i}(a)=1$ if $\#{j:a_{j}=Revolt} >= 2,000,000$
  - $u_{i}(a)=-1$ if $\#{j:a_{j}=Revolt} < 2,000,000$ and $a_{i}=Revolt$
  - $u_{i}(a)=0$ if $\#{j:a_{j}=Revolt} < 2,000,000$ and $a_{i}=Not$
...
Players have **exactly opposed** interests + There must be precisely two
players (otherwise they can’t have exactly opposed interests) + For all
action profiles \$a A, u\_{1}(a)+u\_{2}(a) =c \$ for some constant $c$ +
Special case: zero sum + Thus, we only need to store a utility function
for one player + in a sense, we only have to think about one player’s
interests
...
Players have **exactly the same** interests + no conflict: all players
want the same things +
$\forall a \in A, \forall i,j, u_{i}(a)=u_{j}(a)$ + we often write such
games with a single payoff per cell + why are such games
“noncooperative”?
...
- Each player names an interger between 1 and 100
- The player who names the integer closest to two thirds of the
  *average* interger wins a prize, the other players get nothing
...
- If $X$ is no more than 67, then the optimal strategy of any player has
  to be no more that $\frac{2}{3} 67$

- If $X$ is no more than $\frac{2}{3} 67$, then the optimal strategy of
  any player has to be no more that $\frac{2}{3}^2 67$
